...
export function Toolbar() {
  ...
  function openTerminal() {
    fsGateway
      .openTerminal(path, terminalId)
      .catch((e) => logger.error("openTerminal failed", e))
  }
  ...
  return (
    <>
      <header
        ...
        className="flex h-12 w-full shrink-0 items-center gap-1 border-b border-border/60 bg-background/95 pr-3 pl-2 backdrop-blur"
      >
      ...
      {diskUsageOpen && (
        <DiskUsagePanel onClose={() => setDiskUsageOpen(false)} />
      )}
    </>
  )
}
